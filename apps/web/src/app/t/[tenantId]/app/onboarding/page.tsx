...
export default function OnboardingPage() {
  ...
  return (
    <div className="mx-auto grid w-full max-w-5xl gap-8 py-4 lg:grid-cols-[0.7fr_1fr] lg:items-center lg:py-12">
      ...
      <div className="auth-form mx-auto w-full max-w-lg rounded-[22px] border border-border bg-white p-6 shadow-[0_18px_50px_rgba(0,75,73,0.08)] sm:p-10">
        ...
        <div className="mb-8 grid grid-cols-4 gap-2" aria-label="Onboarding progress">
          {steps.map((item, index) => (
            <div key={item} className={`h-1.5 rounded-full ${index <= stepIndex ? "bg-[var(--auth-lime)]" : "bg-black/10"}`} />
          ))}
        </div>
        ...
        {step === "property" ? (
          <>
            ...
            <div className="mt-8 grid gap-4">
              <div className="flex min-h-28 items-center justify-center rounded-[16px] border border-dashed border-teal/30 bg-[var(--auth-mint)] text-center text-xs text-muted">Add a property photo<br />Optional</div>
        ...
        {step === "trust" ? (
          <>
            ...
            <div className="mt-8 grid gap-3">
              <TrustRow label="Payment history" value="Build your record over time" />
              <TrustRow label="Tenancy history" value="Add verified experience" />
              <TrustRow label="Profile completeness" value="Keep your details current" />
            </div>
          </>
        ) : null}
        ...
        {step === "notifications" ? (
          <>
            <StepHeading title={copy.title} description={copy.description} />
            <div className="mt-8 grid gap-3">
              <Toggle label="Payment reminders" checked={notifications.payments} onChange={() => setNotifications({ ...notifications, payments: !notifications.payments })} />
              <Toggle label="Messages and invitations" checked={notifications.messages} onChange={() => setNotifications({ ...notifications, messages: !notifications.messages })} />
              <Toggle label="Product updates" checked={notifications.updates} onChange={() => setNotifications({ ...notifications, updates: !notifications.updates })} />
            </div>
          </>
        ) : null}
        ...
        {step === "complete" ? (
          <>
            <StepHeading title={copy.title} description={copy.description} />
            <div className="mt-8 grid gap-3 rounded-[16px] bg-[var(--auth-mint)] p-4 text-sm">
              <div className="flex justify-between"><span className="text-muted">Workspace profile</span><span className="font-semibold text-success">Complete</span></div>
              <div className="flex justify-between"><span className="text-muted">Notifications</span><span className="font-semibold text-success">Configured</span></div>
            </div>
          </>
        ) : null}
        ...
        <div className="mt-8 flex gap-3">
          ...
          {step === "complete" ? (
            <Link className="flex-1" href={`/t/${tenantId}/app`}>
              <Button className="w-full bg-[var(--auth-lime)] text-teal hover:bg-[#a9eb00]">Go to dashboard</Button>
            </Link>
          ) : (
            <Button className="flex-1 bg-[var(--auth-lime)] text-teal hover:bg-[#a9eb00]" onClick={next}>Continue</Button>
          )}
        </div>
...
function TrustRow({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[14px] border border-border p-4"><div className="flex justify-between gap-4 text-sm font-semibold"><span>{label}</span><span className="text-xs font-bold text-success">In progress</span></div><div className="mt-1 text-xs text-muted">{value}</div></div>;
}

function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return <button type="button" onClick={onChange} className="flex items-center justify-between rounded-[14px] border border-border p-4 text-left text-sm font-medium"><span>{label}</span><span className={`h-5 w-9 rounded-full p-0.5 transition-colors ${checked ? "bg-[var(--auth-lime)]" : "bg-black/15"}`}><span className={`block h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${checked ? "translate-x-4" : "translate-x-0"}`} /></span></button>;
}
