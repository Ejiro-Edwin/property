...
type Dashboard = { activeTenancies: number; totalProperties?: number; occupiedProperties?: number; vacantProperties?: number; occupiedPropertyIds?: string[]; paymentSummary: { totalPayments: number; onTime: number; late: number; missed: number; collectedAmount?: number; expectedAmount?: number; collectionRate?: number }; propertyPerformance?: { month: string; collectedAmount: number; expectedAmount: number }[]; notifications: string[] };
...
export default function OverviewPage() {
  ...
  React.useEffect(() => {
    ...
    api<{ payments: Payment[] }>("payments/history", { tenantId, query: { limit: 100 } }).then((r) => setPayments(r.payments ?? [])).catch(() => setPayments([]));
  ...
  const performanceMax = Math.max(...performance.flatMap((item) => [item.collectedAmount, item.expectedAmount]), 1);
  const currentHour = new Date().getHours();
  const greeting = currentHour < 12 ? "Good morning" : currentHour < 18 ? "Good afternoon" : "Good evening";
  const outstandingPayments = (payments ?? []).filter((payment) => ["pending", "late", "missed", "partial"].includes(payment.status.toLowerCase()));
  const pendingAmount = outstandingPayments.filter((payment) => ["pending", "partial"].includes(payment.status.toLowerCase())).reduce((total, payment) => total + payment.amount, 0);
  const overdueAmount = outstandingPayments.filter((payment) => ["late", "missed"].includes(payment.status.toLowerCase())).reduce((total, payment) => total + payment.amount, 0);

  return (
    <div className="mx-auto grid w-full max-w-[1140px] gap-4 pb-8">
      <section className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-[2rem] font-semibold tracking-[-0.045em] text-[#24211f]">{greeting}, {sessionUser.name?.split(" ")[0] ?? "landlord"}.</h1>
      ...
      <section className="rounded-[8px] border border-[#dfe7e3] bg-white p-5 shadow-[0_4px_16px_rgba(15,23,42,0.03)]"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold text-[#24211f]">Rent Collection</h2><p className="mt-1 text-xs text-[#77716d]">Overview of rent collection this cycle</p></div><Link href={`/t/${tenantId}/app/payments`} className="text-xs font-semibold text-[#45863b]">View payments</Link></div><div className="mt-4 grid gap-4 sm:grid-cols-4"><div><div className="text-[11px] text-[#77716d]">Collected</div><div className="mt-1 font-bold text-[#45863b]">{formatMoney(portfolioRent)}</div></div><div><div className="text-[11px] text-[#77716d]">Pending</div><div className="mt-1 font-bold text-[#d08a28]">{formatMoney(pendingAmount)}</div></div><div><div className="text-[11px] text-[#77716d]">Overdue</div><div className="mt-1 font-bold text-[#d95c4e]">{formatMoney(overdueAmount)}</div></div><div className="sm:col-span-1"><div className="h-2 rounded-full bg-[#e8f0e3]"><div className="h-2 rounded-full bg-[#baff00]" style={{ width: `${collectionRate}%` }} /></div><div className="mt-2 text-right text-[10px] text-[#77716d]">{collectionRate}%</div></div></div></section>

      <div className="grid gap-5 lg:grid-cols-[1.35fr_0.9fr]"><section><div className="mb-2 flex items-center justify-between"><h2 className="text-sm font-semibold text-[#24211f]">Outstanding Payments</h2><Link href={`/t/${tenantId}/app/payments`} className="text-xs font-semibold text-[#45863b]">View all</Link></div><div className="rounded-[8px] border border-[#dfe7e3] bg-white p-4">{payments === null ? <Skeleton className="h-16" /> : outstandingPayments.length === 0 ? <div className="py-4 text-center text-sm text-[#77716d]">No outstanding payments</div> : outstandingPayments.slice(0, 3).map((payment) => <div key={payment.id} className="flex items-center justify-between border-b border-[#edf0ed] py-3 last:border-0"><div><div className="text-sm font-semibold text-[#24211f]">Payment due</div><div className="text-xs text-[#77716d]">{formatDateTime(payment.dueDate ?? payment.createdAt)}</div></div><Badge tone={paymentStatusTone(payment.status)}>{payment.status}</Badge></div>)}</div></section><section><div className="mb-2 text-sm font-semibold text-[#24211f]">Quick Actions</div><div className="grid grid-cols-2 gap-2"><QuickAction href={`/t/${tenantId}/app/properties/add`} label="Add Property" /><QuickAction href={`/t/${tenantId}/app/people/add`} label="Add Tenant" /><QuickAction href={`/t/${tenantId}/app/tenancies/create`} label="Create Tenancy" /><QuickAction href={`/t/${tenantId}/app/payments/record`} label="Record Payment" /></div></section></div>

      <section><div className="mb-2 flex items-center justify-between"><h2 className="text-sm font-semibold text-[#24211f]">Your Properties</h2><Link href={`/t/${tenantId}/app/properties`} className="text-xs font-semibold text-[#45863b]">View all properties</Link></div><div className="grid gap-4 sm:grid-cols-3">{properties === null ? <Skeleton className="h-36" /> : properties.length === 0 ? <div className="col-span-full rounded-[8px] border border-[#dfe7e3] bg-white p-8 text-center text-sm text-[#77716d]">No properties yet. Add your first property to get started.</div> : properties.slice(0, 3).map((property) => { const occupied = dashboard?.occupiedPropertyIds?.includes(property.id) ?? false; return <Link key={property.id} href={`/t/${tenantId}/app/properties/${property.id}`} className="rounded-[8px] border border-[#dfe7e3] bg-white p-3 shadow-[0_4px_16px_rgba(15,23,42,0.03)]"><div className="property-art h-24 rounded-[6px]" /><div className="mt-3 text-sm font-semibold text-[#24211f]">{property.title}</div><div className="mt-1 text-xs text-[#77716d]">{property.address}</div><div className="mt-2 flex justify-between text-xs"><span>{formatMoney(property.rentAmount, property.currency)}</span><Badge tone={occupied ? "success" : "warning"}>{occupied ? "Occupied" : "Vacant"}</Badge></div></Link>; })}</div></section>

      <section><div className="mb-2 flex items-center justify-between"><div><h2 className="text-sm font-semibold text-[#24211f]">Property Performance</h2><p className="mt-1 text-[10px] text-[#71817e]">Collected rent compared with expected monthly rent</p></div><div className="flex gap-3 text-[10px] text-[#71817e]"><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-[#baff00]" />Collected</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-[#004b49]" />Expected</span></div></div><div className="flex h-[170px] items-end justify-around rounded-[8px] border border-[#dfe7e3] bg-white px-4 pb-5 pt-4 shadow-[0_4px_16px_rgba(15,23,42,0.03)]">{dashboard === null ? <Skeleton className="h-full w-full" /> : performance.length === 0 ? <div className="w-full self-center text-center text-sm text-[#77716d]">No performance data yet</div> : performance.map((item) => <div key={item.month} className="flex h-full flex-col items-center justify-end gap-2"><div className="flex h-[108px] items-end gap-1"><span className="w-3 rounded-t bg-[#baff00]" title={`${item.month} collected: ${formatMoney(item.collectedAmount)}`} style={{ height: `${Math.max(3, (item.collectedAmount / performanceMax) * 108)}px` }} /><span className="w-3 rounded-t bg-[#004b49]" title={`${item.month} expected: ${formatMoney(item.expectedAmount)}`} style={{ height: `${Math.max(3, (item.expectedAmount / performanceMax) * 108)}px` }} /></div><span className="text-[10px] text-[#71817e]">{item.month}</span></div>)}</div></section>
